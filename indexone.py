...
def login_session_url(username,password,login_url):
    ...
    return parser
# 跳转到俱乐部app，开始抢俱乐部
def get_login_url_cookie(parser,login_url,headers,jlb_url,username):
    # 拿到ck
    ...
    if hd['datas']['getHdyyData']['totalSize'] != 0 or hd['datas']['getHdyyData']['totalSize'] != None:
        for i in hd['datas']['getHdyyData']['rows']:
            ...
            makeapphd = session.post('https://ehall.sziit.edu.cn/qljfwapp/sys/szxxtyjlbapp/hdyy/makeAppointmentHd.do',params=par).json()
            # code返回值如果不等于-1则可能已经预约成功

            if makeapphd['code'] != '-1':
                # 可能已经抢到了
                try:
                    print((datetime.datetime.now()).strftime("%Y-%m-%d, %H:%M:%S"),'活动标题:{}'.format(i['HDMC']),makeapphd['msg'])
                except KeyError:
                    ...
                    title = i['HDMC']
                    print((datetime.datetime.now()).strftime("%Y-%m-%d, %H:%M:%S"),'活动标题:{}已成功预约,正在发送通知邮件'.format(i['HDMC']))
...
def e_mail(receviers,title,username):
    ...
    mail_user = '1374205781@qq.com'  # 用户名00
    mail_pass = 'bnrurlsidofzjdcb'  # smtp登录口令
    ...
    message.attach(MIMEText('{} 用户{}俱乐部活动{}已被抢到'.format((datetime.datetime.now()).strftime("%Y-%m-%d, %H:%M:%S"),username,title)))  # 邮件正文内容
...
def auto_jlb():
    ...
    for users in config['users']:
        ...
        get_login_url_cookie(parser,login_url,headers,jlb_url,username)

def main():
    d_time = datetime.datetime.strptime(str(datetime.datetime.now().date()) + '12:15', '%Y-%m-%d%H:%M')
    d_time1 = datetime.datetime.strptime(str(datetime.datetime.now().date()) + '19:00', '%Y-%m-%d%H:%M')
